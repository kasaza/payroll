...
<div class="page-head">
    <ol class="breadcrumb">
		...
		<a href="pdf"  class="btn btn-danger right" target="_blank" class="fl_right" title="Print the Official Report"><i class="fa fa-print fa-fw"></i> Official Report</a>
    </ol>
</div>
<!-------MAIN CONTENT------>
		...
		<table id="payroll" class="table table-bordered table-striped">
			<!--<table id="payroll" class="display wrap table">-->
			  ...
			  <tr id="<?php echo $row["p_id"]; ?>" > 
					...
					<td><?php echo' <a class="btn btn-black btn-xs" target="_blank"  href="print_slip.php?pdf=1&id='.$row["p_id"].'">PDF</a> '?></td>
					<td>
					<div class="btn-group">
	...
	<div id="add_data_Modal" class="modal fade" role="dialog">  
		...
		/*===============Payroll Transactions Calculator================*/
			  $(document).ready(function(){
				...
				//$('#all0,#all1,#all2,#all3').on('keyup', function(){cal_final_total(count);});
				$('#all0,#all1,#all2,#all3').on('blur', function(){cal_final_total(count);});
				//$('#ded0,#ded1,#ded2,#ded3').on('keyup', function(){cal_final_total(count);});
...
/*========Insert Report Data===========*/
	function createReport(){
	   ...
	   $.ajax({  
			...
			/*beforeSend:function(){  
				$('#create_report').val("Re"); 
				$("#create_report").attr("disabled", "disabled");
				
			setInterval(function(){
				$("#create_report").removeAttr("disabled");
				$('#create_report').val('Save Report'); 
				},62208*10^6);
			 },  */
			 success:function(data){
				  $('#report_form')[0].reset();  
